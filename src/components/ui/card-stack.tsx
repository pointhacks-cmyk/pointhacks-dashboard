...
export default function CardStack() {
  ...
  return (
    <div className={`w-full h-screen flex items-center justify-center ${currentTheme.bg} transition-all duration-500 relative overflow-hidden`}>
      ...
      {/* Navigation Buttons */}
      <motion.button onClick={moveToStart} className={`absolute left-8 top-1/2 -translate-y-1/2 p-4 rounded-full ${currentTheme.controlBg} border ${currentTheme.toggleBorder} backdrop-blur-sm transition-colors duration-200 z-20`} whileHover={{ scale: 1.1, x: -5 }} whileTap={{ scale: 0.9 }}>
        <ChevronLeft className={`w-6 h-6 ${currentTheme.text}`} />
      </motion.button>
      <motion.button onClick={moveToEnd} className={`absolute right-8 top-1/2 -translate-y-1/2 p-4 rounded-full ${currentTheme.controlBg} border ${currentTheme.toggleBorder} backdrop-blur-sm transition-colors duration-200 z-20`} whileHover={{ scale: 1.1, x: 5 }} whileTap={{ scale: 0.9 }}>
      ...
      <div className={`absolute bottom-8 left-8 right-8 text-center px-6 py-4 rounded-xl border ${currentTheme.infoBox} backdrop-blur-md transition-all duration-300 z-20 shadow-lg`}>
        <p className={`${currentTheme.text} text-sm font-medium`}>
          Drag up/down · Navigate · Shuffle · Reset
        </p>
        <p className={`${currentTheme.textSecondary} text-xs mt-1`}>
          Card {currentIndex + 1} of {initialCards.length} · {isDark ? 'Dark' : 'Light'} Mode
        </p>
      </div>
    </div>
  );
